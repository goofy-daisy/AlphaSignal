...
AlphaSignal is a local, open-source investment research platform that synthesises price momentum, news sentiment, regulatory filings, and social signals into a single, unified analytical interface. The entire platform operates locally on your machine, leveraging free APIs and open-weight models to ensure complete data privacy and zero ongoing platform costs.
...
AlphaSignal provides a working production reference architecture for this problem. It aggregates these distinct pipelines locally, allowing users to run advanced deep learning models, maintain a semantic research index, and orchestrate an LLM analytical agent entirely on consumer-grade hardware without exposing proprietary watchlists to external cloud providers.
...
| :--- | :--- |
| **yfinance** | Ingests baseline historical OHLCV pricing and matches structured ticker headlines. |
| **ta** | Computes 40 separate technical indicators across trend, momentum, volume, and volatility. |
| **pytorch-forecasting** | Implements the Temporal Fusion Transformer (TFT) for specialised time-series forecasting. |
| **transformers (FinBERT)** | Runs local sequence classification to extract financial sentiment polarity from text. |
| **sentence-transformers** | Maps unstructured corporate reports and text into dense 384-dimensional vector spaces. |
| **faiss-cpu** | Executes high-performance local vector similarity lookups for the RAG architecture. |
| **lightgbm** | Functions as the final tabular meta-learner, blending individual analytical scores. |
| **langchain** | Manages tool registration, input routing, and memory states for the analyst agent. |
| **Ollama (llama3.1)** | Powers the local language inference model to synthesise data metrics into prose. |
| **PostgreSQL** | Serves as the relational storage layer for unified asset metrics, signals, and cache. |
| **FastAPI** | Exposes high-throughput REST endpoints tracking generated scores, assets, and histories. |
| **Streamlit** | Delivers the responsive interactive frontend application interface. |

---

## Known Limitations

System transparency is essential for reliable quantitative analysis. The platform operates with the following design and technical boundaries:

* **Forecasting Volatility Fallback:** The Temporal Fusion Transformer (TFT) is trained on approximately 4 years of daily intervals. For assets displaying structural fragmentation or low volume regimes, walk-forward Information Coefficients can break down. In these scenarios, the model gracefully returns a fallback score of `0.0` rather than interpolating low-confidence trends.
* **ASX Filing Horizons:** The ASX ingestion engine tracks real-time corporate action boards and returns the most recent announcements (typically 5 to 20 items per asset). It does not construct deep multi-year structural archives for regional data.
...
* **Social Feed Density:** By utilising Alpha Vantage and NewsAPI exclusively, social sentiment indicators are highly robust but optimised primarily for major US large-cap equities. For less conversational or thinly traded symbols, the social signal vector automatically falls back to general news text trends.
* **News Corpus Gaps:** Regional or asset-specific tickers (such as AZJ.AX and QAN.AX) can experience sparse coverage across generalised public NewsAPI feeds. For these tickers, sentiment scoring defaults strictly to raw historical headline strings from alternative data passes.
* **Local Agent Generation:** Running complex language tasks on consumer hardware using `llama3.1` can occasionally lead to non-deterministic string returns or raw JSON block escaping. While internal parsing exception catchers filter out most anomalies, generation behaviour may vary across environments.
* **Hardware Compute Bounds:** Due to rigid numerical dependencies within historical versions of the underlying time-series frameworks, inference and model passes evaluate via CPU threads. This step incurs a brief computational layout delay of a few seconds per evaluated asset ticker during calculation runs.
...
Execute background tracking updates and optimise model training layers. *Note: Running the initial time-series training loop requires multiple processing hours; it is recommended to let this run continuously in a decoupled shell session.*
